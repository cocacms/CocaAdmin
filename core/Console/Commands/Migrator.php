...
class Migrator
{
    ...
    public function rollback($paths = [], array $options = [])
    {
        ...
        $migrations = $this->getMigrationsForRollback($options,$paths);
     ...
     * @return array
     */
    protected function getMigrationsForRollback(array $options,array $paths)
    {
        $files = $this->getMigrationFiles($paths);
        $files = Collection::make($files)->map(function($f){
            return $this->getMigrationName($f);
        });

        $ran = $this->repository->getRan();

        $allCanRollback = Collection::make($ran)
            ->filter(function ($file) use ($files) {
    ...
    protected function rollbackMigrations(array $migrations, $paths, array $options)
    {
        ...
        foreach ($migrations as $migration) {
            ...
            if (! $file = Arr::get($files, $migrationObj->migration)) {
                continue;
            }
